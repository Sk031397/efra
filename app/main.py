...
def start_agent_session(session_id,is_audio=False):
    ...
    # create speech config w/voice settings
...
async def agent_to_client_messaging(
    ...
    while True:
        async for event in live_events:
            if event is None:
                continue
            # if the turn complete or interrruped, send it
            ...
            if not isinstance(part,types.Part):
                continue
            # Only send text if it's a partial response
            ...
            # if audio, send base64 audio data
            is_audio = (
                part.inline_data
...
async def client_to_agent_messaging(
    ...
    while True:
        ...
        data = message["data"]
        role = message.get("role","user") # default to 'user'  if user role is not provided
        ...
        if mime_type == "text/plain":
            ...
            # send the audio data 
            # handling is done with ADK automatically
            live_request_queue.send_realtime(
                types.Blob(data=decoded_data,mime_type=mime_type)
            )
            print(f"[CLIENT TO AGENT]: audo/pcm: {len(decoded_data)} bytes")
        else:
